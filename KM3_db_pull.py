...
conn = fdb.connect(dsn='JA:E:\New folder\FoersterTechnik\KM3\DSPKM.FDB', user='sysdba',password='masterkey') 
#cursor = conn.cursor()
...
tablelist = ['DSP$UPDATE', 'KM_BGRUPPE', 'KM_DRUCK', 'KM_KALB', 'KM_KF_TERMIN',
             ...
             'KM_TEMPERATUR', 'KM_TRAENKE_ALARM', 'KM_VERZEHR']

for iter in tablelist:
    tempdf = pd.read_sql("SELECT * FROM {}".format(iter), conn)
    tempdf.to_sql(iter, dbcon, if_exists = 'append', index = False)


tablelist = ['KM_TEMPERATUR', 'KM_TRAENKE_ALARM', 'KM_VERZEHR']
